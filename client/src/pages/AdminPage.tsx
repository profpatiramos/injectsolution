...
export default function AdminPage() {
  ...
  const sync = trpc.bling.sync.useMutation({
    onSuccess: result => { toast.success(`${result.imported} pedido(s) novo(s) importado(s); ${result.skipped} já existentes ou ignorados.`); bling.refetch(); utils.orders.list.invalidate(); utils.dashboard.overview.invalidate(); },
    onError: error => toast.error(error.message),
  });
  ...
  return <div className="mx-auto max-w-6xl enter-up space-y-6">
    ...
    <section className="surface-card overflow-hidden">
      ...
      {bling.data?.status === "CONECTADO" && <div className="flex items-center justify-between border-t border-[#ececef] bg-[#fafafa] px-5 py-3"><p className="text-xs text-[#77777f]"><Check className="mr-1 inline text-emerald-600" size={14} />Conexão pronta para importar pedidos. Sincronize os produtos na página Produtos.</p><button onClick={() => disconnect.mutate()} className="text-xs font-extrabold uppercase tracking-wide text-[#e31937]">Desconectar</button></div>}
    </section>
  </div>;
}
